...
a = arduino('COM3','UNO')
while 1
%     prompt = 'Which finger to move of 1-5\n';
%     X = input(prompt);
%     disp( '8 - down and 2 - up ; Exit - 0');
%         W = input(':'); 
    X = 1;
...
%         end  
        ...
        plot3([A(2,1) A(2,2)] ,[2,2],[B(2,1) B(2,2)],'LineWidth',10,'Color','y');  
        plot3([A(2,2) A(2,3)] ,[2,2],[B(2,2) B(2,3)],'LineWidth',8,'Color','y');
        plot3([A(2,3) A(2,4)] ,[2,2],[B(2,3) B(2,4)],'LineWidth',7,'Color','y');

        plot3([A(3,1) A(3,2)] ,[3,3],[B(3,1) B(3,2)],'LineWidth',10,'Color','r');  
        plot3([A(3,2) A(3,3)] ,[3,3],[B(3,2) B(3,3)],'LineWidth',8,'Color','r');
        plot3([A(3,3) A(3,4)] ,[3,3],[B(3,3) B(3,4)],'LineWidth',7,'Color','r');

        plot3([A(4,1) A(4,2)] ,[4,4],[B(4,1) B(4,2)],'LineWidth',10,'Color','g');  
        plot3([A(4,2) A(4,3)] ,[4,4],[B(4,2) B(4,3)],'LineWidth',8,'Color','g');
        plot3([A(4,3) A(4,4)] ,[4,4],[B(4,3) B(4,4)],'LineWidth',7,'Color','g');
